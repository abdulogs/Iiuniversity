...
if (!isset($_POST) and $_SERVER['REQUEST_METHOD'] == !"POST") {
  ...
  $enrollments = crud::columnsmore(["p.Start_Time", "p.End_Time", "sy.reg_time", "sy.drop_time"]);
  ...
  if ($enrollments) {
    foreach ($enrollments as $enrollment) { ?>
      <tr>
        ...
        <td class="align-middle text-break" data-label="Period">
          <?php echo $enrollment["Start_Time"] . " - " . $enrollment["End_Time"] . " - " . $enrollment["DayOfWeek"]; ?>
        </td>
        <td class="align-middle text-break" data-label="Grade"> <?php echo $enrollment["GRADE"]; ?></td>
        <td class="align-middle px-3" scope="row" data-label="Controls">


          <?php if ($_SESSION["role"] == "Admin") : ?>
            <a class="updateBtn fa fa-pencil mr-1 font-16 text-success" data-id="<?php echo $enrollment["id"]; ?>" href="javascript:void(0)" title="Updaate"></a>
          ...
          <?php elseif ($_SESSION["role"] == "Student") : ?>
            <?php if (!$hold) : ?>
              <?php if (strtotime(date("m/d/Y")) <= strtotime($enrollment["reg_time"]) || strtotime(date("m/d/Y")) <= strtotime($enrollment["drop_time"])) : ?>
          ...
          <?php elseif ($_SESSION["role"] == "Student") : ?>
            <?php if (!$hold) : ?>
              <?php if (strtotime(date("m/d/Y")) <= strtotime($enrollment["drop_time"])) : ?>
