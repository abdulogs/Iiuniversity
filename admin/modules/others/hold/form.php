...
if (!empty($_POST['id'])) {
  ...
  $hold = crud::select()::columns(["holdID", "HoldType"])::table("hold");
  $hold = crud::where(["holdID" => $id])::execute();
  $hold = crud::fetch("one");
    ...
    <form class="modal-content border-0 shadow-lg" id="update">
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group col-sm-6 mb-0">
            <label class="font-weight-bolder mb-0" for="id">ID</label>
            <input class="form-control" type="text" value="<?php echo $hold['holdID']; ?>" disabled />
            <input name="id" type="hidden" value="<?php echo $hold['holdID']; ?>" />
          </div>
          <div class="form-group col-sm-6 mb-0">
            <label class="font-weight-bolder mb-0" for="name">Name</label>
            <input class="form-control" id="name" name="name" type="text" value="<?php echo $hold['HoldType']; ?>" required />
          </div>
        </div>
      </div>
      <div class="modal-footer bg-light">
