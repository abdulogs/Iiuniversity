...
if (!empty($_POST['id'])) {
  ...
  $minor = crud::select()::columns(["MinorID", "Name", "DeptID"])::table("minor")::where(["MinorID" => $id]);
  $minor = crud::execute();
    ...
    <form class="modal-content border-0 shadow-lg" id="update">
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group col-sm-4 mb-2">
            <label class="font-weight-bolder mb-0" for="id">ID</label>
            <input class="form-control font-12" type="text" value="<?php echo $minor['MinorID']; ?>" disabled />
            <input name="id" type="hidden" value="<?php echo $minor['MinorID']; ?>" />
          </div>
          <div class="form-group col-sm-4 mb-2">
            <label class="font-weight-bolder mb-0" for="name">Name</label>
            <input class="form-control font-12" id="name" name="name" type="text" value="<?php echo $minor['Name']; ?>" required />
          </div>
          <div class="form-group col-sm-4 mb-2">
            <label class="font-weight-bolder mb-0" for="department">Department</label>
            <select class="custom-select font-12" id="department" name="department" required>
              ...
              if ($departments) {
                foreach ($departments as $department) { ?>
                  <option <?php echo ($department["DeptID"] == $minor["DeptID"]) ? "selected" : ""; ?> value="<?php echo $department["DeptID"] ?>"><?php echo $department["Name"]; ?></option>
              <?php }
              } ?>
            </select>
          </div>
