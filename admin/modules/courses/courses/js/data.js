...
<td><div class="skeleton-loader" style="border-radius:5px;width:100%;height:10px;"></div></td>
  `;
    ...
    $(document).ready(function() {
      function loadData(page) {
        $.ajax({
          url: "modules/courses/courses/data.php",
          method: "POST",
          data: {
            page: page
          },
  ...
  $(document).on('click', '.createBtn', function(e) {
    $.ajax({
      url: "modules/courses/courses/form.php",
      type: "POST",
      cache: false,
      beforeSend: function() {
        $("#loading-bar").show();
  ...
  $(document).on('submit', '#create', function(e) {
    e.preventDefault();
    $.ajax({
      url: "modules/courses/courses/create.php",
      type: 'post',
  ...
  $(document).on("submit", "#update", function(e) {
    e.preventDefault();
    $.ajax({
      url: "modules/courses/courses/update.php",
      type: 'post',
      data: new FormData(this),
      contentType: false,
      processData: false,
  ...
  $(document).on('click', '.deletebBtn', function(e) {
    if (confirm("Do you really want to delete this!")) {
      var id = $(this).data('id');
      $.ajax({
        url: "modules/courses/courses/delete.php",
        type: 'post',
        data: {
          id: id
        },
