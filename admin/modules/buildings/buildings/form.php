...
if (!empty($_POST['id'])) {
  ...
  $building = crud::select()::columns(["BuildingID", "Name"])::table("building");
  $building = crud::where(["BuildingID" => $id])::execute();
  $building = crud::fetch("one");
  if ($building) { ?>
    <form class="modal-content border-0 shadow-lg" id="update">
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group col-sm-6 mb-2">
            <label class="font-weight-bolder mb-0" for="id">ID</label>
            <input class="form-control font-12" type="text" value="<?php echo $building['BuildingID']; ?>" disabled />
            <input name="id" type="hidden" value="<?php echo $building['BuildingID']; ?>" required />
          </div>
          <div class="form-group col-sm-6 mb-2">
            <label class="font-weight-bolder mb-0" for="name">Name</label>
            <input class="form-control font-12" id="name" name="name" type="text" value="<?php echo $building['Name'] ?>" required />
          </div>
        </div>
      </div>
      <div class="modal-footer bg-light">
